-- Fix get_top_cited_domains and get_top_cited_urls to allow service-role (admin) calls.
-- auth.uid() is NULL when using the service-role key, so skip the ownership check in that case.
